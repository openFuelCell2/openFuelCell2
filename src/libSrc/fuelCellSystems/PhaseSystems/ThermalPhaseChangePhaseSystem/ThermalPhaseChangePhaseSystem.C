...
Foam::ThermalPhaseChangePhaseSystem<BasePhaseSystem>::heatTransfer
(
    volScalarField& T,
...
) const
{
    autoPtr<fvScalarMatrix> eqnPtr =
        BasePhaseSystem::heatTransfer(T, cellMap);

    const word& continuous = this->continuous();
    ...
    )
    {
        if (this->wMDotL_.found(phasePairIter.key()))
        {
            ...
            if 
            (
                pair.phase1().name() != continuous ||
                pair.phase2().name() != continuous
            )
            {
                continue;
            }
            ...
            (
                ...
                T.mesh(),
                dimensionedScalar("wMDotL", dimEnergy/dimTime, 0.0)
            );
